...
export class FishAudioService {
  ...
  static async generateAudioWithBuffer(
    ...
    const { text, format = 'mp3', reference_id, radioEffects: _radioEffects, ...restOptions } = options;
    ...
    console.log(`[FishAudioService] Requesting format: ${format}`);
    ...
    console.log(`[FishAudioService] Received ${audioBuffer.length} bytes from Fish Audio (format: ${format})`);
    console.log(`[FishAudioService] Returning ${format} audio`);
    return {
      buffer: audioBuffer,
